        ...
        queries = {
            ...
            "9. Sales With Branch Name": """
                SELECT cs.sale_id, cs.name, cs.product_name, cs.gross_sales,
                       cs.status, b.branch_name
                FROM customer_sales cs JOIN branches b ON cs.branch_id=b.branch_id
            """,
            "10. Branch-wise Total Gross Sales": """
                SELECT b.branch_name, SUM(cs.gross_sales) AS total_gross
                FROM branches b JOIN customer_sales cs ON b.branch_id=cs.branch_id
                GROUP BY b.branch_name
            """,
            "11. Sales With Payment Method": """
                SELECT cs.sale_id, cs.name, cs.gross_sales,
                       ps.payment_method, ps.amount_paid
                FROM customer_sales cs JOIN payment_splits ps ON cs.sale_id=ps.sale_id
            """,
            "12. Pending Amount > 5000": """
                SELECT sale_id, name, gross_sales, received_amount, pending_amount
                FROM customer_sales WHERE pending_amount > 5000
            """,
            "13. Top 3 Highest Gross Sales": """
                SELECT sale_id, name, gross_sales
                FROM customer_sales ORDER BY gross_sales DESC LIMIT 3
            """,
            "14. Branch With Highest Sales": """
                SELECT b.branch_name, SUM(cs.gross_sales) AS total_sales
                FROM branches b JOIN customer_sales cs ON b.branch_id=cs.branch_id
                GROUP BY b.branch_name ORDER BY total_sales DESC LIMIT 1
            """,
            "15. Payment Method-wise Total Collection": """
